 ...
 * 
 * This class demonstrates two approaches: - Brute Force using a TreeSet. -
 * Optimal solution using two-pointer technique.
 *
 * Time Complexity: - Brute Force: O(n log n) + O(n) — inserting elements into a
 * TreeSet. - Optimal: O(n) — single linear pass through the array.
 *
 * Space Complexity: - Brute Force: O(n) — extra space for the TreeSet. -
...
public class RemoveDuplicatedSortedArray_2 {
	...
	public RemoveDuplicatedSortedArray_2() {
	 ...
	 * 
	 * Utilizes a TreeSet to store unique elements in sorted order, and overwrites
	 * the original array.
	 *
	...
	/**
	 * Removes duplicates from a sorted array using the optimal two-pointer
	 * approach.
	 * 
